...
- `tests/OutputReportCases.json`: 登録済み79ケースの入力 SQL と和名定義
- `tests/SqlAnalysisFormatter.OutputExpectations.xlsx`: 登録済み79ケースの期待値ブック
...
`run-output-golden-tests.ps1` は79ケースについてセル値、主要罫線、塗り、フォント、折り返し、縮小表示、行高、列幅、目盛り線を実 Excel で比較します。内訳はユーザーレビュー済み76件、暫定期待値3件です。
