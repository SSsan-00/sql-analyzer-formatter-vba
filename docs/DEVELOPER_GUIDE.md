...
- `tests/OutputReportCases.json`: 登録済み81ケースの入力 SQL と和名定義
- `tests/SqlAnalysisFormatter.OutputExpectations.xlsx`: 登録済み81ケースとレビュー待ちケースの期待値ブック
...
`run-output-golden-tests.ps1` はユーザーレビュー済み81ケースについてセル値、主要罫線、塗り、フォント、折り返し、縮小表示、行高、列幅、目盛り線を実 Excel で比較します。
