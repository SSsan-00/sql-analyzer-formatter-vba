...
`run-output-golden-tests.ps1` は77ケースについてセル値、主要罫線、塗り、フォント、折り返し、縮小表示、行高、列幅、目盛り線を実 Excel で比較します。内訳はユーザーレビュー済み74件、暫定期待値3件です。SEL-078は実装済みですが、レビュー確定までは登録済み期待値77件へ含めません。
